...
func NewOrderService() (*OrderService, error) {
	...
	if err := service.connectToServices(); err != nil {
		return nil, err
	}
...
func (s *OrderService) connectToServices() error {
	...
	if err != nil {
		...
		if s.inventoryConn != nil {
			if closeErr := s.inventoryConn.Close(); closeErr != nil {
				log.Printf("Failed to close inventory connection during cleanup: %v", closeErr)
			}
		}
